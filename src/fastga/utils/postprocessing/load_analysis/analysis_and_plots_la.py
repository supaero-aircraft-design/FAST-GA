...
def fuselage_bending_diagram(
    ...
    horizontal_bending_vector = list(
        variables["data:loads:fuselage:horizontal_bending_vector"].value
    )
    ...
    horizontal_bending_inertia = list(
        variables["data:loads:fuselage:horizontal_bending_inertia"].value
    )
    ...
    scatter = go.Scatter(
        x=x_vector,
        y=horizontal_bending_vector,
        mode="lines",
        name=name + " fuselage horizontal bending moment",
    )
    fig.add_trace(scatter, 1, 1)

    scatter = go.Scatter(
        x=x_vector,
        y=vertical_bending_vector,
        mode="lines",
        name=name + " fuselage vertical bending moment",
    )
    fig.add_trace(scatter, 1, 1)

    scatter = go.Scatter(
        x=[x_vector[0], x_vector[-1]],
        y=[shell_bending_inertia, shell_bending_inertia],
        mode="lines",
        name=name + " shell bending inertia",
    )
    fig.add_trace(scatter, 2, 1)

    scatter = go.Scatter(
        x=x_vector,
        y=horizontal_bending_inertia,
        mode="lines",
        name=name + " sollicitated horizontal bending inertia",
    )
    fig.add_trace(scatter, 2, 1)

    scatter = go.Scatter(
        x=x_vector,
        y=vertical_bending_inertia,
        mode="lines",
        name=name + " sollicitated vertical bending inertia",
    )
    fig.add_trace(scatter, 2, 1)
