...
def time_modules(
    config_dictionary=None, ORIGINAL_CONFIGURATION_FILE=None, WORK_FOLDER_PATH=None, problem=None
):

    from tests.testing_utilities import get_indep_var_comp, list_inputs, run_system

    NEW_SOURCE_FILE = pth.join(
        "src/fastga/command/module_sequencing_optimizer/data/oad_process_outputs.xml"
    )
    ...
    ###################### WARNING, THIS IS INCOMPLETE AND DOES NOT WORK YET (TIMING OF INDIVIDUAL MODULES WHEN RUNNING FROM A PROBLEM INSTEAD OF A CONFIG FILE)
        ...
        for module in modules_in_problem:
            ...
            inputs = get_indep_var_comp(
                list_of_variables,
                "src/fastga/command/module_sequencing_optimizer/",
                "oad_process_outputs.xml",
            )
