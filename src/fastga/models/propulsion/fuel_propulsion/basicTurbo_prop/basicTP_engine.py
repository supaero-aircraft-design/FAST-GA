...
class BasicTPEngine(AbstractFuelPropulsion):
    def __init__(
        ...
        :param c: percentage of the total aspirated airflow used for turbine cooling
        ...
        self.c = c  # Percentage of the total aspirated airflow used for turbine cooling
    ...
    def turboshaft_performance_solver_real_gas(self, var_to_solve, mc, p_2t, t_2t, m_air):
        ...
        f[0] = (
            mc * self.eta_q
            - m * (1 + f_fuel_ratio - g - self.c - icb) * (cp_4 * t_4t - cp_3 * t_3t)
        ) / 1e6
        f[1] = (
            self.a41 - m * (1 + f_fuel_ratio - g - icb) * np.sqrt(t_41t * rg) / p_41t / f_gamma_41
        ) * 1e3
        # Pressure change through the compressors
        f[2] = (p_3t - p_25t * (t_3t / t_25t) ** (f1_25 * self.eta_253)) / 1e5
        f[3] = opr_2 / opr_1 - self.opr_2_opr_1_dp
    ...
    def exhaust_mach_solver_real_gas(self, t_5t, t_45t, p_45t, p_0):
        ...
        function_minimize = t_5t - t_45t * (p_5t / p_45t) ** (f2_45 * self.eta_455)
    ...
    def turboshaft_performance_real_gas(self, h, flight_mach, mc):
        ...
        v_8 = mach_8 * np.sqrt(gamma5 * rg * t_8)
    ...
    def turboshaft_compute_within_limits(self, target_power, h, mach_vol):
        ...
        if t_45t_sol > self.itt_limit:
            fuel = self.turboshaft_performance_envelope_limits_real_gas(
                "t_45t", self.itt_limit, h, mach_vol
            )
