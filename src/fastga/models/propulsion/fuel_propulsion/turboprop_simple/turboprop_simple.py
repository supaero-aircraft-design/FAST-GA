...
class GasTurbine(object):
    ...
    def __init__(self, aircraft, generator_usage=False):
        ...
        self.generator_usage = generator_usage  # True if the GT is used as a generator, False if used as an engine
    ...
    def compute_power_max(self, altitude=0.0, V_TAS=0.0):
        ...
        p_to = self.aircraft.vars_sizing_mission['p_takeoff'] * 1000.
        ...
        delta_0 = pression / pression_SL * ( 1. + ( gamma - 1.0 ) / 2.0 * Mach ** ( 2.0 ) ) ** ( gamma / ( gamma - 1.0 ) )
    ...
    def compute_fuel_consumption(self, alt, power, throttle=1.0):
        ...
        PSFC = 3.447e-14 * alt**2 - 2.359e-9 * alt + 1.298e-4  # regression from TBM 900 Pilot Instruction Manual
    ...
    def change_power(self, power):
        ...
        """
        if self.generator_usage:
            self.aircraft.vars_propulsion_hybrid['generator_max_power'] = power / 1000.  # in kW
        else:
            self.aircraft.vars_propulsion_gas_turbine['gas-turbine_max_power'] = power / 1000.  # in kW
