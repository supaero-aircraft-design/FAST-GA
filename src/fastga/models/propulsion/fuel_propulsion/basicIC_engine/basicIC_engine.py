...
class BasicICEngine(AbstractFuelPropulsion):
    ...
    def __init__(
        ...
        volume = self.max_power/np.max(pme_limit_vect * 1e5 * rpm_vect / 240.0) #  conversion rpm to rad/s included
        self.volume = volume
    ...
    def read_map(map_file_path):
        ...
        sfc_matrix = np.zeros((len(np.array([i for i in sfc_lines[0].split(' ') if i != ''])), len(sfc_lines)))
    ...
    def sfc(
        ...
        :param thrust: Thrust (in N)
        :parame engine_setting: Engine settings (climb, cruise,... )
        :param atmosphere: Atmosphere instance at intended altitude
        ...
        if np.size(engine_setting) == 1:
            rpm_values = self.rpm_values[engine_setting]
            mixture_values = self.mixture_values[engine_setting]
        else:
            rpm_values = np.array([self.rpm_values[engine_setting[idx]] for idx in range(np.size(engine_setting))])
            mixture_values = np.array([self.mixture_values[engine_setting[idx]] for idx in range(np.size(engine_setting))])
        ...
        if np.size(thrust) == 1:
            ...
            for idx in range(np.size(thrust)):
                local_atmosphere = Atmosphere(atmosphere.get_altitude()[idx], altitude_in_feet=False)
                local_atmosphere.mach = atmosphere.mach[idx]
                real_power[idx] = thrust[idx] * atmosphere.true_airspeed[idx] / \
    ...
    def max_thrust(
        ...
        if np.size(engine_setting) == 1:
            rpm_values = np.array(self.rpm_values[float(engine_setting)])
        else:
            rpm_values = np.array([self.rpm_values[engine_setting[idx]] for idx in range(np.size(engine_setting))])
