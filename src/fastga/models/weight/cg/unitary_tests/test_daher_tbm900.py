...
from tests.testing_utilities import setup_and_run_system
...
def test_compute_cg_engine():
    ...
    # Research independent input value in .xml file
    problem = setup_and_run_system(ComputeEngineCG(), __file__, XML_FILE)
    x_cg_b1 = problem.get_val("data:weight:propulsion:engine:CG:x", units="m")
    assert x_cg_b1 == pytest.approx(2.23, abs=1e-2)
...
def test_compute_cg_fuel_propulsion():
    ...
    # Research independent input value in .xml file
    problem = setup_and_run_system(FuelPropulsionCG(), __file__, XML_FILE)
...
def test_compute_cg_passenger_seats():
    ...
    assert x_cg_d2 == pytest.approx(5.43, abs=1e-2)  # modified with new cabin definition
...
def test_compute_cg_ratio_aft():
    ...
    # Research independent input value in .xml file and add values calculated from other modules
    problem = setup_and_run_system(ComputeCGRatioAircraftEmpty(), __file__, XML_FILE)
...
def test_compute_cg_load_case():
    ...
    assert mac_max == pytest.approx(0.449, abs=1e-3)
    mac_min = problem["data:weight:aircraft:CG:ground_condition:min:MAC_position"]
    assert mac_min == pytest.approx(0.312, abs=1e-2)
    ...
    assert mac_max == pytest.approx(0.483, abs=1e-3)
    mac_min = problem["data:weight:aircraft:CG:flight_condition:min:MAC_position"]
    assert mac_min == pytest.approx(0.292, abs=1e-2)
...
def test_complete_cg():
    ...
    # with data from file
    problem = setup_and_run_system(CG(propulsion_id=ENGINE_WRAPPER), __file__, XML_FILE)
