...
from ...constants import SUBMODEL_CL_BETA_VT


@oad.RegisterSubmodel(
    SUBMODEL_CL_BETA_VT, "fastga.submodel.aerodynamics.vertical_tail.roll_moment_beta.legacy"
)
class ComputeClBetaVerticalTail(om.ExplicitComponent):
    """
    ...
    holds. The convention from :cite:`roskampart6:1990` are used, meaning that for lateral
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        if self.options["low_speed_aero"]:
            ...
            ] = (z_v * np.cos(aoa_ref) - lp_vt * np.sin(aoa_ref)) / wing_span
            partials[
                "data:aerodynamics:vertical_tail:low_speed:Cl_beta",
            ...
            partials[
                ...
                0.5 * cy_beta_vt * np.cos(aoa_ref) / wing_span
            )
            partials[
                "data:aerodynamics:vertical_tail:low_speed:Cl_beta",
            ...
            partials[
                ...
                -cy_beta_vt * (z_v * np.cos(aoa_ref) - lp_vt * np.sin(aoa_ref)) / wing_span ** 2.0
            )
            partials[
                "data:aerodynamics:vertical_tail:low_speed:Cl_beta",
                "settings:aerodynamics:reference_flight_conditions:AOA",
