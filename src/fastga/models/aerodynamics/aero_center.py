...
class ComputeAeroCenter(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem(
            "comp_aero_center_stick_fixed", ComputeAeroCenterStickFixed(), promotes=["*"],
        )
        self.add_subsystem(
            "comp_aero_center_free_elevator_factor", ComputeAeroCenterFreeElevatorFactor(), promotes=["*"],
        )
        self.add_subsystem(
            "comp_aero_center_stick_free", ComputeAeroCenterStickFree(), promotes=["*"],
        )


class ComputeAeroCenterStickFixed(ExplicitComponent):
    """Aerodynamic center estimation - Stick Fixed"""
...
class ComputeAeroCenterFreeElevatorFactor(ExplicitComponent):
    """Aerodynamic center estimation - Free Elevator Factor"""
...
class ComputeAeroCenterStickFree(ExplicitComponent):
    """Aerodynamic center estimation - Stick Free"""
    ...
    def setup(self):
        ...
        self.add_input("data:aerodynamics:fuselage:cm_alpha", val=np.nan, units="rad**-1")
        self.add_input("data:aerodynamics:horizontal_tail:efficiency", val=np.nan)
        self.add_input("data:aerodynamics:cruise:neutral_point:free_elevator_factor", val=np.nan)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        cm_alpha_fus = inputs["data:aerodynamics:fuselage:cm_alpha"]
        tail_efficiency = inputs["data:aerodynamics:horizontal_tail:efficiency"]
        free_elevator_factor = inputs["data:aerodynamics:cruise:neutral_point:free_elevator_factor"]
