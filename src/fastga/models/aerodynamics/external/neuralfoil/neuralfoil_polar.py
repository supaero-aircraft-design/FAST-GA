...
class NeuralfoilPolarNoAlpha(om.Group):
    """Runs a polar computation with Neuralfoil and returns 2D aerodynamic coefficients."""
    ...
    def setup(self):
        ...
        airfoil_path = Path(airfoil_folder_path) / airfoil_file
        airfoil_path = self._create_temp_airfoil_file(airfoil_path)


        self.add_subsystem("beta", _BetaCompressibilityCorrection(), promotes=["*"])
...
class _BetaCompressibilityCorrection(om.ExplicitComponent):
    """Compressibility correction factor for Neuralfoil polar computation."""

    def setup(self):
        self.add_input("mach", val=np.nan)

        self.add_output("beta", val=np.nan)

    def setup_partials(self):
        self.declare_partials(of="beta", wrt="mach", method="exact")

    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        outputs["beta"] = np.sqrt(1.0 - inputs["mach"]**2.0)

    def compute_partials(self, inputs, partials, discrete_inputs=None):
        partials["beta", "mach"] = -inputs["mach"] / np.sqrt(1.0 - inputs["mach"]**2.0)
