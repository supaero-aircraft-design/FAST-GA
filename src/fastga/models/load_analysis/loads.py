...
class Loads(om.Group):
    ...
    def setup(self):
        self.add_subsystem(
            ...
            AerostructuralLoad(compute_cl_alpha=True, use_openvsp=self.options["use_openvsp"],),
            promotes=["*"],
        )
        self.add_subsystem(
            "structural_loads", StructuralLoads(compute_cl_alpha=True), promotes=["*"]
        )
        self.add_subsystem(
            "aerodynamic_loads", AerodynamicLoads(compute_cl_alpha=True), promotes=["*"]
        )
