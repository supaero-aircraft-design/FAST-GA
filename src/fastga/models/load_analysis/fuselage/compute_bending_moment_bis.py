...
class ComputeBendingMomentBis(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("data:loads:fuselage:cone_mass", val=np.nan, units="kg")
        self.add_input("data:loads:fuselage:nose_mass", val=np.nan, units="kg")
        ...
        self.add_input("data:loads:fuselage:additional_mass", val=np.nan, units="kg")
        ...
        self.add_output("data:loads:fuselage:horizontal_bending_inertia", units="m**4", shape=FUSELAGE_MESH_POINT)
        self.add_output("data:loads:fuselage:vertical_bending_inertia", units="m**4", shape=FUSELAGE_MESH_POINT)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        cone_mass = inputs["data:loads:fuselage:cone_mass"]
        nose_mass = inputs["data:loads:fuselage:nose_mass"]
        fuselage_shell_mass = inputs["data:loads:fuselage:shell_mass"]
        bending_inertia = inputs["data:loads:fuselage:inertia"]
        sigma_mh = inputs["data:loads:fuselage:sigmaMh"]
        airframe_add_mass = inputs["data:loads:fuselage:additional_mass"]
        ...
        # Assumption made that this weight is applied over the cabin but not the nose or the cone.
        ...
        for load_case in load_case_array:
            ...
            # Calculation of the horizontal bending moment on the rear of the fuselage
            ...
            index = np.where(horizontal_bending_vector_rear * fuselage_radius / sigma_mh - bending_inertia * e_skin / e_stringer > 0)[0]
            if index.size == 0:
                volume_rear = 0.0
            else:
                ...
                if x_h_bend <= l_front + l_cabin:
                    ...
                    a2_rear = n * x_ratio_centroid_to_cabin_rear / (l_cabin * fuselage_radius * sigma_mh) * \
                    ...
                    volume_rear = a2_rear * ((l_front + l_cabin - wing_centroid)**3 - (l_front + l_cabin - x_h_bend)**3) / 3 \
                                  + a1_rear * ((tail_x_cg - wing_centroid)**2 - (tail_x_cg - x_h_bend)**2) / 2 \
                    ...
                    a1_rear = (n * w_tail + rmh * lift_max_h) / (fuselage_radius * sigma_mh)
                    a0_rear = - bending_inertia / (fuselage_radius**2 * e_stringer / e_skin)
                    volume_rear_cabin = a2_rear * (l_front + l_cabin - wing_centroid)**3 / 3 \
                                  + a1_rear * ((tail_x_cg - wing_centroid)**2 - (tail_x_cg - (l_front + l_cabin))**2) / 2 \
                                  + a0_rear * (x_h_bend - wing_centroid)
                    volume_rear_tail = a1_rear * ((tail_x_cg - (l_front + l_cabin))**2 - (tail_x_cg - x_h_bend)**2) / 2 \
                                       + a0_rear * (x_h_bend - (l_front + l_cabin))
            ...
            max_moment_front = n * w_nose * wing_centroid + \
                               n * x_ratio_centroid_to_cabin_front / l_cabin * \
                               (w_payload + w_padd_uniform + w_cabin + w_airframe_add + w_insulation + w_seats) * \
            ...
            for x in x_vector_front:
                if engine_layout == 3:
                    if x <= engine_cg_x:
                        bending = n * w_nose / l_front * x ** 2 + moment_to_compensate_with_lift / wing_centroid * x
                    elif x <= l_front:
                        bending = n * w_nose / l_front * x ** 2 + moment_to_compensate_with_lift / wing_centroid * x + n * engine_mass * (x - engine_cg_x)
                    else:
                        bending = n * w_nose * x + n * engine_mass * (x - engine_cg_x) + \
                                  n * x_ratio_centroid_to_cabin_front / l_cabin * \
                                  (w_payload + w_padd_uniform + w_cabin + w_airframe_add + w_insulation + w_seats) * \
                                  (x - l_front) ** 2 \
            ...
            # Calculation of x_h_bend
            index = np.where(horizontal_bending_vector_front * fuselage_radius / sigma_mh - bending_inertia * e_skin / e_stringer > 0)[0]
            ...
            if index.size == 0:
                ...
                if x_h_bend <= l_front:
                    ...
                    a1_front_nose = moment_to_compensate_with_lift / (wing_centroid * fuselage_radius * sigma_mh)
                    a2_front_nose = n * w_nose / (l_front * fuselage_radius * sigma_mh)
                    a1_front_cabin = (n * w_nose + moment_to_compensate_with_lift / wing_centroid) /\
                                     (fuselage_radius * sigma_mh)
                    a2_front_cabin = n * x_ratio_centroid_to_cabin_front / l_cabin * \
                                     (w_payload + w_padd_uniform + w_cabin + w_airframe_add + w_insulation + w_seats) /\
                                     (fuselage_radius * sigma_mh)
                    volume_front_nose = a0_front * (l_front - x_h_bend) + a1_front_nose / 2 * (l_front - x_h_bend) ** 2\
                                        + a2_front_nose / 3 * (l_front - x_h_bend) ** 3
                    volume_front_cabin = a0_front * (wing_centroid - l_front) + a1_front_cabin / 2 * (wing_centroid - l_front) ** 2\
                    ...
                    a2_front_cabin = n * x_ratio_centroid_to_cabin_front / l_cabin * \
                                     (w_payload + w_padd_uniform + w_cabin + w_airframe_add + w_insulation + w_seats) /\
                                     (fuselage_radius * sigma_mh)
                    volume_front = a0_front * (wing_centroid - x_h_bend) + a1_front_cabin / 2 * (wing_centroid - x_h_bend) ** 2\
                                   + a2_front_cabin / 3 * ((wing_centroid - l_front) ** 3 - (x_h_bend - l_front) ** 3)
            ...
            if n == n_lift:
                horizontal_bending_vector_plot = np.append(horizontal_bending_vector_front, horizontal_bending_vector_rear)

        # Computation of the vertical additional mass
        ...
        index = np.where(vertical_bending_vector * fuselage_radius / sigma_mh - bending_inertia * e_skin / e_stringer < 0)[0]
        ...
        if index.size == 0:
            ...
            volume = b1 * ((tail_x_cg - wing_centroid)**2 - (tail_x_cg - x_v_bend)**2) / 2 + b0 * (x_v_bend - wing_centroid)
        additional_mass_vertical = volume * rho_skin

        # Total fuselage airframe mass
        airframe_mass = fuselage_shell_mass + airframe_add_mass + additional_mass_horizontal + additional_mass_vertical

        # Arrays to plot
        vertical_bending_vector_plot = np.append(np.zeros(len(x_vector_front)), vertical_bending_vector)
        horizontal_bending_inertia_plot = horizontal_bending_vector_plot * fuselage_radius / sigma_mh
